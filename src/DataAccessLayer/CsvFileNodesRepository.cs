...
namespace RoutesBetweenStations.DataAccess
{
    ...
    public class CsvFileNodesRepository : INodesRepository
    {
        ...
        public CsvFileNodesRepository()
            : this(new TrainStationFactory(), new TrainConnectionFactory(), new FileStreamProvider())
        {
        }
        ...
        public CsvFileNodesRepository(INodeFactory nodeFactory, INodeConnectionFactory nodeConnectionFactory, IFileStreamProvider fileStreamProvider)
        {
        ...
        {
            using (var stream = _FileStreamProvider.GetCsvFile())
            {
                ...
                using (var reader = new StreamReader(stream, System.Text.Encoding.UTF8, true, 128, true))
                {
                    int lineNumber = 0;
